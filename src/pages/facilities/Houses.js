...
export default function Houses() {
    ...
    return (
        <div className='bg-white py-[40px] md:py-[80px] lg:py-[100px]'>
            <div className="container sm:container md:container lg:max-w-[1232px] px-4 mx-auto" id="houses">
                ...
                <div className="flex flex-wrap -mx-4 lg:-mx-[11px]">
                    {houseData && houseData?.map((item, index) => (
                        <div key={index} className="w-full sm:w-6/12 lg:w-3/12 px-4 lg:px-[11px] mb-5 lg:mb-0 text-center ">
                            ...
                            <h2 className={`merriweather-font font-normal text-xl md:text-2xl tracking-[-0.04em] mb-3 lg:mb-4 ${item.color}`}>{item.name}</h2>
                            <p className="text-sm lg:text-base text-[#666666] tracking-[-0.04em] font-medium">{item.paragraph}</p>
                        </div>
                    ))}
